# this script creates a node graph for chr1, chr2, chr3, chr4 of the ESC data
...
dim(rawESCdata)
str(rawESCdata)
# we can see this is a 394 x 16 dataframe
# start by isolating the relevant data for chr1
chr1 <- rawESCdata[2:length(rawESCdata[,2]), 2:4]

head(chr1)
# TAD number in 1st + 2nd columns with their interaction in 3d column
str(chr1)
# note that chr1 is a data.frame with `character' entries. We want them to be numeric
chr1$X <- as.numeric(chr1$X)
chr1$X.1 <- as.numeric(chr1$X.1)
chr1$X.2 <- as.numeric(chr1$X.2)
# I think we can do all at once with: chr1 = transform(A, X = as.numeric(X), X.1 = as.numeric(X.1), X.2 = as.numeric(X.2))

# now to clarify the names of each column
...
# given the max/min values of the TADs available, we will want a 394x394 matrix
...
ESCchr1.mapper <- mapper1D(distance_matrix = chr1dist)
...
########################################################        
# Ok. we get a node graph with the default values. Let's see how different the node graphs look for stem cells vs neurons
########################################################        

# Let's start by looking at node graphs for all the stem cell chromosome data we have
########################################################        
##############         chr2             ################
########################################################        
...
ESCchr2.mapper <- mapper1D(distance_matrix = chr2dist)
ESCchr2.graph <- graph.adjacency(ESCchr2.mapper$adjacency, mode = "undirected")
...
ESCchr3.mapper <- mapper1D(distance_matrix = chr3dist)
ESCchr3.graph <- graph.adjacency(ESCchr3.mapper$adjacency, mode = "undirected")
...
ESCchr4.mapper <- mapper1D(distance_matrix = chr4dist)
ESCchr4.graph <- graph.adjacency(ESCchr4.mapper$adjacency, mode = "undirected")
