...
chr2 = rawESCdata[['start: t##.1', 'end: t##.1', 'interaction.1']]
...
for i in range(len(chr2)):
    dist2.set_value(chr2['startTAD'][i], chr2['endTAD'][i], chr2['distance'][i])
    
dist2 = dist2.sort_index(axis = 1,ascending = True)
dist2 = dist2.sort_index(axis = 0,ascending = True)
...
for i in dist2.index.values:
  for j in dist2.columns.values:
    if dist2.loc[i,j] <d:
      graph.append([i,j])
